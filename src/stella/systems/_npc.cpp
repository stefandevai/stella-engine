...
namespace system
{
  NPC::NPC (entt::registry& registry, entt::entity player_entity) : System ("npc"), m_player_entity (player_entity)
  {
    ...
    if (registry.valid (m_player_entity))
    {
      ...
      if (!player_speech.messages.empty())
      {
        ...
        if (!player_message.read)
        {
          ...
          // each one of them
          registry.view<component::NPC, component::Position>().each ([&registry, player_pos, this, player_message] (auto entity, auto& npc, auto& pos) {
            ...
            if (distx < m_distx && disty < m_disty)
            {
              ...
              registry.emplace<component::Timer> (
                  response, 3000.0, 0.0, component::Timer::Type::DECREASE, [] (entt::registry& r, const entt::entity e) { r.destroy (e); }, false);
      ...
      // each one of them
      const auto& player_pos = registry.get<component::Position> (m_player_entity);
      registry.group<component::NPC> (entt::get<component::Position, component::Body2D>).each ([this, player_pos] (auto entity, auto& npc, auto& pos, auto& body) {
        const float total_dist = std::sqrt (std::pow (player_pos.x - pos.x, 2) + std::pow (player_pos.y - pos.y, 2));
