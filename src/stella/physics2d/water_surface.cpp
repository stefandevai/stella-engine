...
namespace physics2d
{
    ...
    void WaterSurface::update(const double dt)
    {
      if (m_active)
      {
        ...
        if (corrected_wave_speed >= m_height/dt) corrected_wave_speed = m_height/dt - 1.0;
        ...
        for (unsigned i = 0; i < m_number_of_columns; ++i)
        {
          ...
          if (i == 0)
          {
            //offset = pow(corrected_wave_speed, 2)*(1.0 + m_U[i+1] - 2.0*m_U[i])/pow(m_height, 2);
            offset = (1.0 + m_U[i+1])/2.0 - 2.0*m_U[i];
          }
          else if (i + 1 >= m_number_of_columns)
          {
             //offset = pow(corrected_wave_speed, 2)*(m_U[i-1] + 1.0 - 2.0*m_U[i])/pow(m_height, 2);
             offset = (m_U[i-1] + 1.0)/2.0 - 2.0*m_U[i];
          }
          else
          {
             //offset = pow(corrected_wave_speed, 2)*(m_U[i-1] + m_U[i+1] - 2.0*m_U[i])/pow(m_height, 2);
    ...
    void WaterSurface::perturbate(const double x, const double amount)
    {
      ...
      if (column - 1 > 0) this->set_column_height(column-1, amount*0.8);
      if (column + 2 < m_number_of_columns) this->set_column_height(column+2, amount*0.4);
      if (column - 2 > 0) this->set_column_height(column-2, amount*0.4);
    }
