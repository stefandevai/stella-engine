...
struct SpriteComponent {
  ...
  SpriteComponent(const std::string &tex_name, const float framex, const float framey, std::string layer_id = "basic", unsigned frame = 0) : TexName(tex_name), FrameDimensions(glm::vec2(framex, framey)), LayerId(layer_id), Frame(frame) { }
  ...
  SpriteComponent(graphics::Texture &texture, const float framex, const float framey, int frame, std::string layer_id = "basic")
