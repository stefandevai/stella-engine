...
private:
  ...
  float m_window_width = 0.f, m_window_height = 0.f, m_game_width = 0.f,
        m_game_height = 0.f;

  // graphics::Texture texture_placeholder{"assets/sprites/player.png"};
  std::vector<glm::vec2> vertices{glm::vec2{0.0f, 20.0f},
                                  glm::vec2{128.0f, 80.0f},
                                  glm::vec2{128.0f, 128.0f},
                                  glm::vec2{0.0f, 128.0f}};
  std::shared_ptr<graphics::Shape> shape =
      std::make_shared<graphics::Shape>(vertices, glm::vec3{200.f, 100.f, 1.f});
  // std::shared_ptr<graphics::Sprite> sprite_placeholder =
  // std::make_shared<graphics::Sprite>(300, 300, 100, 100, texture_placeholder);
...
public:
  ...
  ~EditorGui();
  void
  init(SDL_Window *window, SDL_GLContext gl_context, const char *glsl_version);
  void configure_input(SDL_Event &event);
  void render(const float window_width,
